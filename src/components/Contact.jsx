  ...
  const handleSubmit = (e) => {
    ...
    emailjs.send (
      ...
      'template_vixljo5',
      {
        ...
        to_email: 'bikashkc2057@gmail.com',
    ...
    <div className='xl:mt-12 xl:flex-row flex-col-reverse flex gap-10 overflow-hidden'>
      <motion.div variants={slideIn('left', 'tween', 0.2, 1)} className='flex-[0.75] bg-black-100 p-8 rounded-2xl'>
        ...
        <form ref={formRef} onSubmit={handleSubmit} className='mt-12 flex flex-col gap-8'>
          ...
          <button type='submit' className='bg-tertiary py-3 px-8 outline-none w-fit text-white font-bold shadow-primary shadow-md rounded-xl hover:bg-tertiary'>
            {loading ? 'Sending...' : 'Send'}
          </button>
        </form>
      </motion.div>

      <motion.div variants={slideIn('right', 'tween', 0.2, 1)} className='xl:flex-1 xl:h-auto ms:h-[550px] h-[350px]' >
        <EarthCanvas />
      </motion.div>
    </div>
  )
